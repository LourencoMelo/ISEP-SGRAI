...
export default class Watch extends THREE.Group {
    constructor(cityName, center = new THREE.Vector2(0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        */
        ...
        let points = [];
        let markerAngle = (2 *Math.PI)/60;

        for(let i=0; i<60;i++){
            let x,y;
            if(i % 5 == 0){
                x = radius0 * Math.cos(markerAngle * i);
                y = radius0 * Math.sin(markerAngle * i);
            }
            else{
                x = radius1 * Math.cos(markerAngle * i);
                y = radius1 * Math.sin(markerAngle * i);
            }
            points.push(new THREE.Vector2(x,y));

            x = radius2 * Math.cos(markerAngle * i);
            y = radius2 * Math.sin(markerAngle * i);
    ...
    update() {
        ...
        */
        angle = Math.PI / 2.0 - 2.0 * Math.PI * (time[1]/60 +time[2]/3600);
        ...
        */
        angle = Math.PI / 2.0 - 2.0 * Math.PI * (time[0] /12 + time[1]/(12*60) + time[2]/(12*3600)) ;
