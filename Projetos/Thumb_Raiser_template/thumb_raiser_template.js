            ...
            if (!this.animations.actionInProgress) {
                ...
                if (this.maze.foundExit(this.player.position)) {
                    ...
                    if (this.player.keyStates.run) {
                        ...
                        ...; */
                        coveredDistance = coveredDistance * this.player.runningFactor;
                        /* To-do #15 - Adjust the player's direction increment
                            - now assume that the player is running:
                            - multiply the direction increment by this.player.runningFactor
                        ...; */
                        directionIncrement = directionIncrement * this.player.runningFactor;
                    ...
                    */
                    if (this.player.keyStates.backward) { // The player is moving backward
                        const newPosition = new THREE.Vector3(coveredDistance* Math.sin(this.player.direction) + this.player.position.x, 0, coveredDistance*Math.sin(this.player.direction) + this.player.position.z).add(this.player.position);
                    ...
                    else if (this.player.forward) { // The player is moving forward
                        const newPosition = new THREE.Vector3(coveredDistance * Math.sin(this.player.direction),0,coveredDistance * Math.sin(this.player.direction)).add(this.player.position);
